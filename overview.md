# Objectives
oViceという2Dのメタバースシステムを使っています。すでにログインするIDを作って利用していますが、これをPlaywrightを使って、自動的にログインすることができないか検討しています。プログラム的にPlaywrightを使って指定されたIDとPasswordを使ってログインさせ、さらに、自動でスピーカーとマイクをオンの状態にし、入ってきた音声入力に対してOpen AIやGeminiなどのAIに音声データを渡し、それに対する返答をやってもらうということをしたいです。どの程度技術的に可能か検討し、実装方法の提案をしてください
